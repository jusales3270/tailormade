...
// pauta.excluir — só quem propôs, e só antes da ata publicada.
export const excluirPauta = actionClient
  .metadata({ acao: "pauta.excluir", entidade: "reuniao_pauta" })
  .inputSchema(schema)
  .action(async ({ parsedInput, ctx, metadata }) => {
    ...
    if (!pautaItem.proposto_por || pautaItem.proposto_por !== membro.id) {
      throw new Error("Só quem propôs este item de pauta pode excluí-lo.");
    }
    ...
    if ((ataCount ?? 0) > 0) {
      throw new Error("Não dá pra excluir: a ata desta reunião já foi publicada.");
    }

    const { error: deleteError } = await supabase.from("reuniao_pauta").delete().eq("id", pautaItem.id);
